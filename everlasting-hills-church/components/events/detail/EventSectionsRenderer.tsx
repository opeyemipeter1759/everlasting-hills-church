...
function EventSectionView({ section, event, index }: { section: EventSection; event: EventDetail; index: number }) {
    ...
    return (
      <section className="bg-[#10080b] px-4 py-20 text-white xs:px-5 sm:px-8 md:py-28">
        <div className="mx-auto max-w-4xl text-center">
          ...
          {section.content.body && <p className="mx-auto mt-6 max-w-2xl whitespace-pre-line text-[17px] leading-[1.7] text-white/65">{section.content.body}</p>}
  ...
  return (
    <section className={`${background} px-4 py-20 xs:px-5 sm:px-8 md:py-28`}>
      <div className="mx-auto max-w-5xl">
        <SectionHeading section={section} />
        {section.type === "RICH_TEXT" && <p className="mx-auto mt-8 max-w-2xl whitespace-pre-line text-center text-[17px] leading-[1.7] text-[#555]">{section.content.body}</p>}
        ...
        {section.content.actions.map((action, actionIndex) => {
          ...
          return (
            <article
              ...
              className="flex flex-col rounded-sm border border-[#E7CDD3] bg-white p-6 xs:p-8"
            >
...
function SectionHeading({ section, dark = false }: { section: EventSection; dark?: boolean }) {
  ...
  return <header className="mx-auto max-w-3xl text-center">{section.subtitle && <p className={`text-[10px] font-semibold uppercase tracking-[0.18em] xs:tracking-[0.22em] ${dark ? "text-[#E7CDD3]" : "text-[#87102C]"}`}>{section.subtitle}</p>}{section.title && <h2 className={`mt-4 text-balance text-3xl font-black tracking-[-0.03em] sm:text-5xl ${dark ? "text-white" : "text-[#111]"}`}>{section.title}</h2>}</header>;
}
...
function ScheduleSection({ section, event }: { section: Extract<EventSection, { type: "SCHEDULE" }>; event: EventDetail }) {
  ...
  return <div className="mt-10"><p className="mx-auto max-w-2xl text-center text-base leading-7 text-[#666]">{section.content.introduction}</p><p className="mt-4 text-center text-xs font-black uppercase tracking-[0.18em] text-[#87102C]">Every day · {formatEventDateRange(event.startAt, event.endAt, event.timezone)}</p><div className="mx-auto mt-10 grid max-w-3xl gap-px overflow-hidden rounded-sm border border-[#E7CDD3] bg-[#E7CDD3] sm:grid-cols-2">{event.Schedules.map((schedule) => <div key={schedule.id} className="bg-white p-6 sm:p-8"><p className="text-3xl font-black tracking-tight text-[#87102C]">{formatClock(schedule.startTime)}</p><h3 className="mt-2 text-lg font-bold text-[#111]">{schedule.title}</h3>{schedule.description && <p className="mt-2 text-sm leading-6 text-[#666]">{schedule.description}</p>}</div>)}</div>{section.content.tags.length > 0 && <ul className="mt-7 flex flex-wrap justify-center gap-2">{section.content.tags.map((tag) => <li key={tag} className="rounded-sm border border-[#E7CDD3] bg-white px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#6E0C24]">{tag}</li>)}</ul>}</div>;
}

function ExpectationsSection({ section }: { section: Extract<EventSection, { type: "EXPECTATIONS" }> }) {
  return <div className="mt-10">{section.content.introduction && <p className="mx-auto mb-8 max-w-2xl text-center leading-7 text-[#666]">{section.content.introduction}</p>}<ol className="grid gap-px overflow-hidden rounded-sm border border-[#E7CDD3] bg-[#E7CDD3] md:grid-cols-2">{section.content.items.map((item, index) => <li key={`${item.title}-${index}`} className="bg-white p-6 sm:p-8"><span className="text-xs font-black text-[#87102C]">{String(index + 1).padStart(2, "0")}</span><h3 className="mt-4 text-xl font-black leading-tight text-[#111]">{item.title}</h3><p className="mt-3 text-sm leading-7 text-[#666]">{item.description}</p>{item.scripture && <p className="mt-4 text-xs font-bold uppercase tracking-wider text-[#87102C]">{item.scripture}</p>}</li>)}</ol></div>;
}
