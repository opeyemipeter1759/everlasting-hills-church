...
/**
 * Branded confirmation dialog. Replaces the browser's native confirm() for any action
 * that is destructive or otherwise needs explicit user buy-in (role change, deletion).
 *
 * Why a custom dialog: native confirm() looks like 1998, blocks the JS event loop, and
 * cannot be styled. This one matches the rest of the dashboard UI and supports an async
 * confirm handler with loading state.
 */
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white dark:bg-[#1c1c1e] border border-gray-200 dark:border-white/10 shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 bg-gray-50 dark:bg-white/5 border-t border-gray-100 dark:border-white/10">
          ...
          <button
            ...
            onClick={() => onConfirm()}
            disabled={loading}
            className={`px-4 py-2 rounded-lg text-sm font-bold text-white transition-colors focus:outline-none focus:ring-2 disabled:opacity-60 disabled:cursor-not-allowed ${styles.button}`}
          >
