...
export default function Modal({ open, onClose, title, description, children, maxWidth = "md" }: ModalProps) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="modal-title" className="fixed inset-0 z-50 flex items-center justify-center px-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} aria-hidden="true" />
      <div className={`relative w-full ${MAX_W[maxWidth]} rounded-2xl bg-white dark:bg-[#1c1c1e] border border-gray-200 dark:border-white/10 shadow-2xl`}>
        {/* Header — overflow-hidden here keeps rounded corners on the top edge only */}
        <div className="flex items-start justify-between gap-3 px-6 pt-5 pb-4 border-b border-gray-100 dark:border-white/8 rounded-t-2xl overflow-hidden">
          <div>
        ...
        {/* Body */}
        <div className="px-6 py-5 rounded-b-2xl overflow-visible">{children}</div>
      </div>
    </div>
  );
}
