...
import { Crown, Heart, Zap, Users, type LucideIcon } from "lucide-react";
...
/** Icons are fixed per group (not editable via the CMS text form). */
...
/** Fallback content, seeded from the CMS `ministries` default. */
export const MINISTRIES: MinistryGroup[] = [
  {
    slug: "mens",
    name: "Men's Ministry",
...
function MinistryCard({ m, inverted }: { m: MinistryGroup; inverted: boolean }) {
  const [flipped, setFlipped] = useState(false);
  const Icon = ICONS[m.slug] ?? Crown;
        ...
        >
          ...
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 ${
              inverted ? "bg-white/15" : "bg-[#87102C]/8"
            }`}
          >
            <Icon size={22} strokeWidth={1.8} className={inverted ? "text-white" : "text-[#87102C]"} />
          </div>

          <h3 className={`text-xl font-bold mb-3 leading-tight ${inverted ? "text-white" : "text-[#111]"}`}>
            {m.name}
          </h3>

          <p className={`text-sm leading-relaxed flex-1 ${inverted ? "text-white/70" : "text-[#555]"}`}>
            {m.body}
          </p>

          <div className={`mt-6 inline-flex self-start items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${
            inverted ? "bg-white/15 text-white/70" : "bg-[#87102C]/8 text-[#87102C]"
          }`}>
            {m.verseRef}
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" aria-hidden>
              <path d="M5 12h14M13 6l6 6-6 6" stroke={inverted ? "#fff" : "#87102C"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity={inverted ? 0.7 : 0.6} />
            </svg>
          </div>
        </div>
...
export default function MinistriesCards({ groups = MINISTRIES }: { groups?: MinistryGroup[] }) {
  return (
    <div className="grid sm:grid-cols-2 gap-5 md:gap-6">
      {groups.map((m, i) => (
        <ScrollReveal key={m.slug} delay={0.1 + i * 0.12}>
          <MinistryCard m={m} inverted={i === 1} />
        </ScrollReveal>
      ))}
    </div>
  );
