...
import { AlertTriangle, ChevronRight, HandHeart, ListChecks, Phone, type LucideIcon } from "lucide-react";
...
function CountRow({
  ...
  return (
    <li>
      <Link
        href={href}
        className={`${shell} w-full transition-colors hover:bg-[#FFE8ED] dark:hover:bg-white/[0.06]`}
      >
        {body}
      </Link>
    </li>
...
/**
 * Shepherding at a glance: what is outstanding, and who specifically needs
 * reaching. The three counts are all open work — prayer requests still pending,
 * follow-ups still open, members drifting — so an all-clear is a real state and
 * says so rather than showing three zeros.
 *
...
export default function PastoralCareCard({
  ...
  const allClear = care.prayerRequests === 0 && care.openFollowUps === 0 && care.atRiskMembers === 0;
      ...
      {allClear ? (
        <div className="flex flex-col items-center gap-1.5 py-8 text-center">
          ...
          <p className="text-xs text-[#8a7e80] dark:text-white/40">
            Every prayer request is prayed over and no one is drifting.
          </p>
        </div>
      ) : (
        <>
          ...
          {named.length > 0 && (
            <div className="mt-4 border-t border-[#E7CDD3]/40 pt-4 dark:border-white/[0.07]">
