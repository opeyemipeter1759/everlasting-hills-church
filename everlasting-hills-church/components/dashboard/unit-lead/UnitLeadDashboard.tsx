...
function MemberPositionPicker({
  ...
  const setPosition = useSetMemberPosition();
      ...
      <div>
        <div className="flex justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              href={`/dashboard/unit-lead/${unitId}/reports`}
              className="flex bg-[#6E0C24] text-[#fff] font-medium text-sm rounded-md px-6 h-[40px] items-center"
            >
              Report
            </Link>
          </div>
          ...
          {showAdd && unit && (
            <AddMemberForm
              ...
              onAdded={async (memberId, isLead) => {
                await addMember(memberId, isLead);
                setShowAdd(false);
              }}
            />
          )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">
                      ...
                      {m.position && (
                        <span className="ml-2 text-[9px] font-bold uppercase tracking-wider text-[#87102C] dark:text-[#e8768a]">
                          {m.position.name}
                        </span>
                      )}
                    </p>
                  ...
                  <MemberPositionPicker
                    unitId={unitId}
                    memberId={m.memberId}
                    currentPositionId={m.position?.id ?? null}
                  />
                  <button
                    type="button"
