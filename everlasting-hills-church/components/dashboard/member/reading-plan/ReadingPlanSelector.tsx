    ...
    <div className="mt-5 flex min-w-0 flex-wrap items-end gap-x-4 gap-y-2 rounded-2xl border border-[#E7CDD3]/60 bg-[#FFF4F6]/50 p-3 dark:border-white/10 dark:bg-white/[0.03] sm:p-4">
      <label className="min-w-0 flex-1 basis-52 text-xs font-semibold text-gray-600 dark:text-white/65">
        ...
        <select
          ...
          onChange={(event) => router.push(readingHref(event.target.value, undefined, page))}
        ...
        >
          {(["ACTIVE", "PAUSED", "COMPLETED"] as const).map((status) => {
            const plans = subscriptions.filter((subscription) => subscription.status === status);
            return plans.length > 0 && (
              <optgroup key={status} label={status === "ACTIVE" ? "Reading now" : status === "PAUSED" ? "Paused" : "Completed"}>
                {plans.map((subscription) => (
                  <option key={subscription.subscriptionId} value={subscription.subscriptionId}>
                    {subscription.plan.title} · {subscription.completedDays}/{subscription.plan.durationDays} read
                  </option>
                ))}
              </optgroup>
            );
