...
function StatTiles({ stats }: { stats: ServiceReportStats }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 [&>*:last-child:nth-child(odd)]:col-span-2 sm:[&>*:last-child:nth-child(odd)]:col-span-1">
      {STAT_TILES.map((t) => (
        <div
          key={t.key}
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={6}
                className="w-full rounded-lg border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 text-base sm:text-xs px-3.5 py-3 outline-none focus:ring-2 focus:ring-[#87102C]/25 resize-y leading-relaxed"
              />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-1 border-t border-[#E7CDD3]/40 dark:border-white/[0.07] mt-1">
              <div className="flex items-center gap-1 rounded-lg border border-gray-200 dark:border-white/10 p-0.5 mt-3 sm:mt-0" role="radiogroup" aria-label="Send via">
                {SENT_VIA_OPTIONS.map((o) => (
                  <button
                    key={o.value}
                    type="button"
                    role="radio"
                    aria-checked={sentVia === o.value}
                    onClick={() => setSentVia(o.value)}
                    className={`flex flex-1 items-center justify-center gap-1.5 px-2.5 py-2 sm:flex-none sm:py-1.5 rounded-md text-[11px] font-bold whitespace-nowrap transition-colors ${
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-1.5 px-4 py-2.5 sm:w-auto sm:py-2 rounded-lg text-xs font-bold text-white bg-[#87102C] hover:bg-[#6E0C24] transition-colors disabled:opacity-50 sm:ml-auto"
              >
