...
import {
  ...
  ArrowUpRight, Search, ChevronDown, ChevronRight,
...
function TypeBadge({ type }: { type: string | null }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full ${
      online ? "bg-purple-50 text-purple-700" : "bg-sky-50 text-sky-700"
    }`}>
...
function InterestBadge({ interest }: { interest: string | null }) {
  if (!interest) return <span className="text-gray-300 text-xs">—</span>;
...
function CreateAccountBtn({
  ...
  if (alreadyMember) {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] bg-green-50 text-green-700 border border-green-200 px-2.5 py-1 rounded-full font-semibold whitespace-nowrap">
        <CheckCircle size={11} /> Member
      </span>
    );
  }
  if (!visitor.email || !visitor.phone) {
    return <span className="text-xs text-gray-300">No email/phone</span>;
  }
    ...
    <>
      ...
      >
        <td className="px-5 py-3.5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {visitor.email && (
                <p className="text-gray-400 text-xs leading-tight truncate max-w-[160px]">
                  {visitor.email}
                </p>
              )}
            </div>
        ...
        <td className="px-5 py-3.5 hidden lg:table-cell">
          <span className="text-xs text-gray-400">{relativeDate(visitor.submittedAt)}</span>
        </td>
        <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
          <CreateAccountBtn visitor={visitor} alreadyMember={alreadyMember} onCreated={onCreated} />
        </td>
        <td className="px-3 py-3.5">
          <div className={`text-gray-400 transition-transform duration-200 ${open ? "rotate-90" : ""}`}>
          ...
          <td colSpan={6} className="px-5 py-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
              ...
              <div className="space-y-2">
                ...
                {visitor.email && (
                  <a href={`mailto:${visitor.email}`} className="flex items-center gap-2 text-gray-700 hover:text-[#87102C] transition-colors">
                    <Mail size={12} className="text-gray-400 flex-shrink-0" />
                    {visitor.email}
                  </a>
                )}
                {visitor.phone && (
                  <a href={`tel:${visitor.phone}`} className="flex items-center gap-2 text-gray-700 hover:text-[#87102C] transition-colors">
              ...
              {/* About */}
              <div className="space-y-2">
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">About</p>
                {visitor.gender && (
                  <span className="flex items-center gap-2 text-gray-500">
                    <span className="text-gray-400">Gender:</span> {visitor.gender}
                  </span>
                )}
                {visitor.occupation && (
                  <span className="flex items-center gap-2 text-gray-500">
                ...
                {visitor.bornAgain && (
                  <span className="flex items-center gap-2 text-gray-500">
                    <span className="text-gray-400">Born again:</span> {visitor.bornAgain}
                  </span>
                )}
              </div>
              ...
              {visitor.howDidYouLearn && (
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">How They Found Us</p>
                  <p className="text-gray-600">{visitor.howDidYouLearn}</p>
                </div>
              )}
            </div>
          </td>
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className="w-full pl-8 pr-3 py-1.5 text-sm rounded-lg border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#87102C]/20 focus:border-[#87102C]/40 transition-all placeholder:text-gray-400"
      />
    </div>
  );
}
...
export default function AdminOverview({
  ...
  const STATS = [
    { label: "Total Members",   value: stats.members,      icon: Users,      bg: "bg-sky-50",       color: "text-sky-600",   href: "/dashboard/members"         },
    { label: "First Timers",    value: stats.visitors,     icon: UserPlus,   bg: "bg-amber-50",     color: "text-amber-600", href: "/dashboard/first-timers"    },
    { label: "Today's Check-ins",value:stats.todayCheckIns,icon: CheckCircle,bg: "bg-emerald-50",   color: "text-emerald-600",href:"/dashboard/attendance"      },
    { label: "Prayer Requests", value: stats.prayers,      icon: Heart,      bg: "bg-[#FFF4F6]",    color: "text-[#87102C]", href: "/dashboard/prayer-requests" },
  ] as const;
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      {/* ── Welcome banner ──────────────────────────────────────────────────── */}
      <div className="relative overflow-hidden bg-white rounded-2xl border border-gray-200 px-6 py-5">
        {/* Decorative background gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-white via-white to-[#FFF4F6] pointer-events-none" />
        <div className="absolute right-0 top-0 h-full w-40 bg-gradient-to-l from-[#FFF4F6] to-transparent pointer-events-none" />
        {/* EHC monogram decoration */}
        ...
        <div className="relative">
          <p className="text-xs font-bold uppercase tracking-widest text-[#87102C]/60 mb-1">
            {getDateString()}
          </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STATS.map(({ label, value, icon: Icon, bg, color, href }) => (
          <Link
            key={label}
            href={href}
            className="group relative bg-white rounded-2xl border border-gray-200 p-5 hover:border-[#E7CDD3] hover:shadow-md transition-all duration-200"
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <ArrowUpRight
                size={15}
                className="text-gray-300 group-hover:text-[#87102C] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-200"
              />
            </div>
            <p className="text-[32px] font-bold text-gray-900 leading-none mb-1.5">
              {value}
            </p>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            <h2 className="font-semibold text-gray-900 text-sm">First Timers</h2>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden text-xs font-semibold">
              {FILTER_TABS.map((tab) => (
                ...
                >
                  {tab.label}
                  <span className={`text-[10px] font-bold ${ftFilter === tab.key ? "text-white/60" : "text-gray-400"}`}>
                    {tab.count}
                  </span>
                </button>
              ))}
            ...
            <Link
              ...
              className="text-xs text-[#87102C] hover:underline font-medium whitespace-nowrap"
            >
              View all
            </Link>
          </div>
        ...
        {filteredVisitors.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <UserPlus size={28} className="text-gray-300 mb-3" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide hidden sm:table-cell">Type</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide hidden md:table-cell">Interest</th>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            <h2 className="font-semibold text-gray-900 text-sm">Recent Members</h2>
            <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide hidden sm:table-cell">Phone</th>
              ...
              <tbody>
                {filteredMembers.map((m) => {
                  ...
                  return (
                    <tr key={m.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/70 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <Avatar name={name} />
                          <div>
                            ...
                            {m.email && (
                              <p className="text-gray-400 text-xs leading-tight truncate max-w-[200px]">
                                {m.email}
                              </p>
                            )}
                          </div>
                      ...
                      <td className="px-5 py-3.5 hidden md:table-cell">
                        <span className="text-xs text-gray-400">{relativeDate(m.joinedAt)}</span>
                      </td>
                    </tr>
                  );
                })}
