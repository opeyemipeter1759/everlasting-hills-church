...
const TABS: { key: AnnouncementFilter; label: string }[] = [
  { key: "ALL", label: "All" },
...
export default function AnnouncementsHeader({
  ...
  onFilterChange: (f: AnnouncementFilter) => void;
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#87102C]/10 dark:bg-[#87102C]/20">
          <Megaphone size={19} className="text-[#87102C] dark:text-[#e8768a]" />
        </span>
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">Announcements</h1>
          <p className="text-xs text-gray-400 dark:text-white/40">Keep the church family in the loop</p>
        </div>
      </div>
