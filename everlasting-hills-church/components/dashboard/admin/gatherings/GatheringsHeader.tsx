...
const TABS: { key: GatheringFilter; label: string }[] = [
  { key: "ALL", label: "All" },
  { key: "ACTIVE", label: "Active" },
  { key: "INACTIVE", label: "Paused" },
];

export default function GatheringsHeader({
  filter,
  onFilterChange,
  counts,
  onNew,
  ...
  onFilterChange: (f: GatheringFilter) => void;
  counts: Record<GatheringFilter, number>;
  onNew: () => void;
}) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#87102C]/10 dark:bg-[#87102C]/20">
          <CalendarClock size={19} className="text-[#87102C] dark:text-[#e8768a]" />
        </span>
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">Gatherings</h1>
          <p className="text-xs text-gray-400 dark:text-white/40">
            Recurring meetings members can join and get reminded about
          </p>
        </div>
      </div>
