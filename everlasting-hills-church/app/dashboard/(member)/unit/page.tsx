...
import { useMyMemberships } from "@/lib/api";

export default function UnitIndexPage() {
  const router = useRouter();
  const { data: units, isLoading } = useMyMemberships();

  useEffect(() => {
    if (units && units.length === 1) {
      router.replace(`/dashboard/unit/${units[0].id}`);
    }
  }, [units, router]);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-6 bg-white dark:bg-[#1c1c1e] border border-dashed border-gray-200 dark:border-white/10 rounded-xl p-12 text-center">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
            Contact an admin or your unit lead to be added to a unit.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">You're on more than one unit — pick one.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {units.map((unit) => (
          <button
            key={unit.id}
            type="button"
            onClick={() => router.push(`/dashboard/unit/${unit.id}`)}
          ...
          >
            <span className="w-9 h-9 rounded-full flex-shrink-0 bg-[#87102C]/10 dark:bg-[#87102C]/20 flex items-center justify-center">
              <Users2 size={16} className="text-[#87102C] dark:text-[#e8768a]" />
            </span>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{unit.name}</p>
              <p className="text-xs text-gray-400 dark:text-gray-500">
                {unit.totalMembers} member{unit.totalMembers !== 1 ? "s" : ""}
              </p>
            </div>
          </button>
        ))}
