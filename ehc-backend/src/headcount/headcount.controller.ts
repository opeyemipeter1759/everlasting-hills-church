 ...
 * gate all live in the injected services.
 */
@ApiTags('headcount')
...
export class HeadcountController {
  constructor(
    ...
    private readonly write: HeadcountWriteService,
  ) {}
  ...
  getForDate(@Query('date') date: string) {
    return this.read.getForDate(date);
  }
  ...
  upsertByDate(
    ...
    return this.write.upsertByDate(date, body, { id: user.userId });
  }
  ...
  getForService(@Param('serviceId') serviceId: string) {
    return this.read.getForService(serviceId);
  }
  ...
  upsert(
    ...
    return this.write.upsert(serviceId, body, { id: user.userId });
  }
  ...
  getToday() {
    return this.read.getTodayHeadcount();
  }
