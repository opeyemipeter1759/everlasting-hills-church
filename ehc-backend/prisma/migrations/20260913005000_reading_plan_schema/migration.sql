-- Ordered promotion of the legacy Bible reading-plan schema.
-- Safe on databases where the historical manual SQL was already applied.
-- 
...
RETURNS TRIGGER AS $$
DECLARE
  old_plan_status "PlanStatus";
  new_plan_status "PlanStatus";
  old_target_plan TEXT;
  new_target_plan TEXT;
BEGIN
  IF TG_TABLE_NAME = 'ReadingPlanDay' THEN
    IF TG_OP <> 'INSERT' THEN
      old_target_plan := OLD."planId";
    END IF;
    IF TG_OP <> 'DELETE' THEN
      new_target_plan := NEW."planId";
    END IF;
  ELSE
    IF TG_OP <> 'INSERT' THEN
      SELECT d."planId" INTO old_target_plan
      FROM "ReadingPlanDay" d
      WHERE d."id" = OLD."planDayId";
    END IF;
    IF TG_OP <> 'DELETE' THEN
      SELECT d."planId" INTO new_target_plan
      FROM "ReadingPlanDay" d
      WHERE d."id" = NEW."planDayId";
    END IF;
  END IF;

  IF old_target_plan IS NOT NULL THEN
    SELECT p."status" INTO old_plan_status
    FROM "ReadingPlan" p WHERE p."id" = old_target_plan;
  END IF;
  IF new_target_plan IS NOT NULL THEN
    SELECT p."status" INTO new_plan_status
    FROM "ReadingPlan" p WHERE p."id" = new_target_plan;
  END IF;

  IF old_plan_status = 'PUBLISHED' OR new_plan_status = 'PUBLISHED' THEN
    -- The reflection prompt is the sole mutable day field after publishing.
    IF TG_OP = 'UPDATE' AND TG_TABLE_NAME = 'ReadingPlanDay' THEN
      IF NEW."id" IS NOT DISTINCT FROM OLD."id"
         AND NEW."planId" IS NOT DISTINCT FROM OLD."planId"
         AND NEW."tenantId" IS NOT DISTINCT FROM OLD."tenantId"
         AND NEW."dayIndex" IS NOT DISTINCT FROM OLD."dayIndex"
         AND NEW."title" IS NOT DISTINCT FROM OLD."title"
         AND NEW."referenceLabel" IS NOT DISTINCT FROM OLD."referenceLabel"
         AND NEW."totalWordCount" IS NOT DISTINCT FROM OLD."totalWordCount"
         AND NEW."estimatedMinutes" IS NOT DISTINCT FROM OLD."estimatedMinutes" THEN
        RETURN NEW;
      END IF;
    END IF;
...
CREATE TRIGGER "ReadingPlanDay_locked_when_published"
  BEFORE INSERT OR UPDATE OR DELETE ON "ReadingPlanDay"
